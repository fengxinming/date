...
function createDateFromArray(arr) {
  const isUTC = arr[7] === 'Z';
  let [year, month, date, hours, minutes, seconds, ms] = arr;
...
function createDateFromString(input, format) {
  if (isString(format)) {
    return createDateFromArray(
      extractFromFormat(input, format)
    );
  } else {
    // 自动判断格式
...
export default function (input, format) {
  let time;
  if (isNil(input)) { // parse()
    input = new Date();
  } else if (isString(input)) { // parse('2020-01-01') or parse('2020-01-01', 'YYYY-MM-DD')
    input = createDateFromString(input, format);
  } else if (isArray(input) && input.length) { // [2020, 0, 1]
    input = createDateFromArray(input);
  } else if (isNumber(input)) { // timestamp
    input = new Date(input);
  } else if ((time = +input)) { // Date、moment
    input = new Date(time);
  } else {
    // input = [], {}, etc
    input = new Date();
